...
(define package-default-cache-mode
  "cached")

(define (cached-package-name package-name)
  f"{package-name}-cached")

(define (validate-package-cache-mode who cache-mode)
  (begin
    (unless (member cache-mode package-cache-modes)
      (raise-user-error who
                        f"cache mode must be postinstall or cached: {cache-mode}")
    ) ; end unless supported cache mode
    cache-mode
  ) ; end begin validate-package-cache-mode
) ; end define validate-package-cache-mode

(define (package-cache-mode-rank cache-mode)
  (begin
    (validate-package-cache-mode 'package-cache-mode-rank cache-mode)
    (if (string=? cache-mode "cached") "2" "1")
  ) ; end begin package cache mode rank
) ; end define package-cache-mode-rank
...
(define (deb-full-release release system [cache-mode package-default-cache-mode])
  (begin
    (validate-package-cache-mode 'deb-full-release cache-mode)
    ; system stays ahead of rank so the unified revision sorts above the legacy
    ; split-name revision {release}.{system} under dpkg comparison at the same
    ; release number; rank stays ahead of the mode word so cached (rank 2)
    ; outranks postinstall (rank 1) for the same release and system
    f"{release}.{system}.{(package-cache-mode-rank cache-mode)}.{cache-mode}"
  ) ; end begin deb-full-release
) ; end define deb-full-release

(define (deb-package-version c release system [cache-mode package-default-cache-mode])
  f"{(cfg-source-version c)}-{(deb-full-release release system cache-mode)}")

(define (deb-generated-package-name c [release (cfg-deb-release c)]
                                    [system (cfg-deb-system c)]
                                    [arch (cfg-deb-arch c)]
                                    [cache-mode package-default-cache-mode])
  f"{(cfg-package-name c)}_{(deb-package-version c release system cache-mode)}_{arch}.deb")
...
`{(cfg-package-name c)}` package: `cached` (the default) embeds the system
...
`{(cached-package-name (cfg-package-name c))}` package is migrated through
...
revision is generated as `deb-release.deb-system.cache-rank.cache-mode`, so `{(cfg-deb-release c)}`
and `{(cfg-deb-system c)}` produce version `{(deb-package-version c (cfg-deb-release c) (cfg-deb-system c) "cached")}`
for the default cached mode and `{(deb-package-version c (cfg-deb-release c) (cfg-deb-system c) "postinstall")}`
...
LEGACY_CACHED_PACKAGE_NAME={(shell-single-quoted (cached-package-name (cfg-package-name c)))}
    ...
    (validate-generated-deb-script! c
                                    ...
                                    '("dpkg-deb --field"
                                      ...
                                      "dpkg-deb --ctrl-tarfile"
	                                      ...
	                                      "DEB does not conflict with the legacy split-name cached package"
	                                      "DEB does not replace the legacy split-name cached package"
	                                      "DEB does not provide the legacy split-name cached package"
	                                      "DEB prerm does not delete the system compiled cache"
...
(define (validate-rpm-spec! c spec-path source-url [cache-mode rpm-default-cache-mode])
  (begin
    ...
    (for ([needle (in-list (list f"%global cache_mode {cache-mode}"
                                 ...
                                 "-U -R \"$runtime_cache_root\" -X \"$runtime_collects_dir\" -G \"$setup_config_dir\" -N raco -l- raco setup"
                                 "--no-launcher"
                                 "runtime_pkgs_dir"
                                 "move_cache_tree"
                                 "runtime-keyed staged system compiled cache"
                                 "runtime-keyed staged package compiled cache"
                                 "racket-compiled-cache.log"
                                 "could not prepare isolated Racket setup config"
                                 "mktemp returned an empty Racket setup config directory"
                                 "isolated setup config did not select the immutable cache root"
                                 "isolated setup config did not select the dynamic cache root"
                                 "setup leaked compiled files into the staged runtime tree"
                                 "rm -rf \"$staged_cache_root\""
                                 "%posttrans"
                                 "cleanup_posttrans"
                                 "rm -rf \"$compiled_cache_root\""
    ...
    (for ([forbidden (in-list (list "AutoReqProv: no"
                                    ...
                                    "rpm -q --quiet"
                                    "-R same"
                                    "setup_bootstrap_root="
                                    "--system"
                                    "--reset-cache"
                                    "--unsafe-delete-all"
                                    "config.rktd.package-racket-final"
...
(define (validate-rpm-ci-workflow! c config path)
  (begin
    ...
    (for ([needle (in-list (list "name: rpm build and release"
                                 ...
                                 "postinstall transaction created a root addon tree"
                                 "postinstall transaction created a HOME addon tree"
                                 "postinstall transaction created a root compiled cache"
                                 "postinstall transaction created a user compiled cache"
    ...
    steps:
      ...
      - name: Build cached portable zip (in-tree compiled)
        ...
        run: |
          ...
          # Some dep entries get recorded as ABSOLUTE build paths when the
          # collects-relative encoder cannot resolve a package collection
          # mid-build (e.g. relative requires into single-collection
          # packages); those bind the tree to the build directory and make
          # cm rebuild them at any other extraction path. Re-encode them
          # post-build with the tree's own racket, where the resolution
          # works; fail the build if anything stays absolute.
          @'
          #lang racket/base
          (require compiler/private/cm-path setup/getinfo racket/path racket/file)
          (define root (simplify-path (path->complete-path (vector-ref (current-command-line-arguments) 0))))
          (define root-str (string-downcase (path->string root)))
          (define pkgs-dir (build-path root \"share\" \"pkgs\"))
          (define rewrites 0)
          (define failures (quote ()))
          (define (under-root? bs)
            (define ps (string-downcase (path->string (bytes->path bs))))
            (and (>= (string-length ps) (string-length root-str))
                 (string=? (substring ps 0 (string-length root-str)) root-str)))
          (define (under-pkgs? p)
            (define rel (find-relative-path pkgs-dir p))
            (and (relative-path? rel)
                 (let ([parts (map path-element->string (explode-path rel))])
                   (and (pair? parts) (not (equal? (car parts) \"..\")) parts))))
          ;; The db in a portable tree lacks sc-pkg-info entries, so the
          ;; stock encoder cannot resolve single-collection packages; read
          ;; the collection from each package info.rkt directly instead.
          (define (encode-pkgs-path p)
            (define parts (under-pkgs? p))
            (and parts (>= (length parts) 2)
                 (let* ([pkgdir (car parts)]
                        [rest (cdr parts)]
                        [info (get-info/full (build-path pkgs-dir pkgdir))]
                        [coll (and info (info (quote collection) (lambda () (quote use-pkg-name))))])
                   (define-values (cname sub)
                     (cond
                       [(eq? coll (quote multi)) (values (car rest) (cdr rest))]
                       [(string? coll) (values coll rest)]
                       [else (values pkgdir rest)]))
                   (and (pair? sub)
                        (cons (quote collects) (map string->bytes/utf-8 (cons cname sub)))))))
          (define (fix-inner v)
            (cond
              [(and (bytes? v) (under-root? v))
               (define enc (path*->collects-relative v))
               (cond
                 [(not (bytes? enc)) (set! rewrites (add1 rewrites)) enc]
                 [(encode-pkgs-path (simplify-path (bytes->path v)))
                  => (lambda (enc2) (set! rewrites (add1 rewrites)) enc2)]
                 [else (set! failures (cons v failures)) v])]
              [else v]))
          (define (fix-dep sexp)
            (cond
              [(and (pair? sexp) (eq? (quote indirect) (car sexp))) (cons (quote indirect) (fix-dep (cdr sexp)))]
              [(and (pair? sexp) (eq? (quote ext) (car sexp))) (cons (quote ext) (fix-inner (cdr sexp)))]
              [else (fix-inner sexp)]))
          (for ([f (in-directory root)])
            (when (and (file-exists? f) (regexp-match? #rx#\"[.]dep$\" (path->bytes f)))
              (define d (call-with-input-file f read))
              (define nd (list* (car d) (cadr d) (caddr d) (map fix-dep (cdddr d))))
              (unless (equal? nd d)
                (call-with-output-file f #:exists (quote truncate)
                  (lambda (o) (write nd o) (newline o))))))
          (printf \"dep re-encodings: ~a\\n\" rewrites)
          (unless (null? failures)
            (for ([b (in-list failures)]) (eprintf \"unencodable dep path: ~a\\n\" b))
            (exit 1))
          '@ | Set-Content -Path normalize-deps.rkt -Encoding UTF8
          & $racketExe normalize-deps.rkt $zipRoot
          if ($LASTEXITCODE -ne 0) {{
            throw \"dep re-encoding failed with exit $LASTEXITCODE\"
          }}
          # Parallel two-pass builds can also leave timestamp inversions (a
          # dependency zo rewritten by pass 2 after its dependent was
          # validated), which cm treats as stale. Normalize like the deb
          # flow: every dep gets one shared timestamp, every zo a slightly
          # later one, so no strict dependency-newer-than-dependent ordering
          # survives.
          $depStamp = Get-Date
          $zoStamp = $depStamp.AddSeconds(2)
          Get-ChildItem -LiteralPath $zipRoot -Recurse -Filter *.dep | ForEach-Object {{ $_.LastWriteTime = $depStamp }}
          Get-ChildItem -LiteralPath $zipRoot -Recurse -Filter *.zo | ForEach-Object {{ $_.LastWriteTime = $zoStamp }}
          # Hard gate: a third setup on the finished tree must be a no-op.
          Start-Sleep -Seconds 3
          $gateMarker = Get-Date
          & $racketExe -N raco -l- raco setup --no-user -D --no-pkg-deps --no-launcher
          if ($LASTEXITCODE -ne 0) {{
            throw \"gate setup failed with exit $LASTEXITCODE\"
          }}
          $gateRebuilt = @(Get-ChildItem -LiteralPath $zipRoot -Recurse -Include *.zo,*.dep | Where-Object {{ $_.LastWriteTime -gt $gateMarker }})
          if ($gateRebuilt.Count -gt 0) {{
            $gateRebuilt | Select-Object -First 20 -ExpandProperty FullName | ForEach-Object {{ Write-Host \"NOT-CONVERGED: $_\" }}
            throw \"cached zip tree did not converge: $($gateRebuilt.Count) rewrites on the builder\"
          }}
          $cachedZipPath = Join-Path \"artifacts\" $env:CACHED_ZIP_NAME
          # bsdtar, not Compress-Archive: full-fidelity mtimes so the shipped
          # in-tree zo/dep pairs revalidate exactly after extraction (DOS
          # 2-second zip timestamps can reorder same-second dep/zo writes)
          & C:\\Windows\\System32\\tar.exe -a -cf $cachedZipPath -C \"portable-zipcached\" $env:PORTABLE_DIR
          if ($LASTEXITCODE -ne 0) {{
            throw \"cached zip creation failed with exit $LASTEXITCODE\"
          }}
		  ...
		  (replace-brew-setup-bootstrap-config-method with-cache-root-directory)
			...
			(define with-no-launcher-cache-setup
			  (regexp-replace* #px"\"--no-pkg-deps\"(?:, \"--no-launcher\")*"
			                   with-setup-bootstrap-config-method
...
(define (print-config c)
  ...
  (when (needs-deb-repo-config? (cfg-targets c))
    ...
    (println/flush f"DEB package version (cached/default): {(deb-package-version c (cfg-deb-release c) (cfg-deb-system c) "cached")}")
    (println/flush f"DEB package version (postinstall/optional): {(deb-package-version c (cfg-deb-release c) (cfg-deb-system c) "postinstall")}")
