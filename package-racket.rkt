...
(define (print-rpm-ci-dry-run-plan! c config)
  (begin
    ...
    (for ([target (in-list targets)])
      (println/flush
       f"  - {(hash-ref target 'rpm-system)} {(hash-ref target 'rpm-arch)} on {(hash-ref target 'runner)} in {(hash-ref target 'container)}")
    ) ; end for target
...
`{zip-name}` as a GitHub Actions artifact. Release asset publishing is
