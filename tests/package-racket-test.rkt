  ...
  (test-case "rpm-ci dry-run validates matrix config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure RPM CI target count: 8")
       (check-contains text "fc43 x86_64 on ubuntu-24.04 in fedora:43")
       (check-contains text "fc44 x86_64 on ubuntu-24.04 in fedora:44")
  ...
  (test-case "rpm-ci installs generated workflow into a temporary rpm repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains workflow-content "EXPECTED_RPM_COUNT: 8")
