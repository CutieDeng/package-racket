...
(define (make-fake-homebrew-tap! base)
  ...
  test do
    assert_match \"9.3.1\", shell_output(\"racket -v\")
  end
end
")
    tap
...
"))
  ...
  (define (write-deb-ci-config! path)
    (write-text!
     path
     "#hash((release-tag . \"v9.3.1\")
      (release-name . \"Racket 9.3.1 DEB packages\")
      (artifact-prefix . \"deb\")
      (create-release . #t)
...
"))
  ...
  (define (write-rpm-ci-config! path)
    (write-text!
     path
     "#hash((release-tag . \"v9.3.1\")
      (release-name . \"Racket 9.3.1 RPM packages\")
      (artifact-prefix . \"rpm\")
      (create-release . #t)
  ...
  (define (write-windows-ci-config! path root #:publish-release? [publish-release? #t])
    (write-text!
     ...
     f"#hash((windows-repo-root . \"{(path-arg root)}\")
      ...
      (release-tag . \"v9.3.1\")
      (release-name . \"Racket 9.3.1 Windows portable\")
      (create-release . #f)
...
(module+ test
  (test-case "production packaging revisions stay synchronized"
    ...
    (check-equal? formula-version (string-append source-version "." expected-release))

    (define deb-repo-config (read-config (build-path package-root "deb-repo-config.rktd")))
    (check-equal? (hash-ref deb-repo-config 'deb-release) expected-release)

    (define deb-ci-config (read-config (build-path package-root "deb-ci-config.rktd")))
    (for ([target (in-list (hash-ref deb-ci-config 'targets))])
      (check-equal? (hash-ref target 'deb-release) expected-release))

    (define rpm-ci-config (read-config (build-path package-root "rpm-ci-config.rktd")))
    (for ([target (in-list (hash-ref rpm-ci-config 'targets))])
      (check-equal? (hash-ref target 'rpm-release) expected-release))
    (check-equal? (hash-ref rpm-ci-config 'release-tag)
                  (string-append "v" source-version "-r" expected-release))

    (define brew-ci-config (read-config (build-path package-root "brew-ci-config.rktd")))
    (check-equal? (hash-ref brew-ci-config 'bottle-rebuild) 1)
  )
  ...
  (test-case "apt dry-run is isolated from apt-release config and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "racket9_9.3.1.1-1_amd64.deb")
  ...
  (test-case "rpm arm64 dry-run normalizes to aarch64 and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Formula/package version: 9.3.1.1")
       (check-contains text "RPM target arch: aarch64")
       (check-contains text "RPM target system: openeuler2403")
       (check-contains text "RPM package version: 9.3.1")
       ...
       (check-contains text "RPM source archive: https://github.com/CutieDeng/racket/releases/download/v9.3.1/racket-minimal-9.3.1-src.tgz")
       (check-contains text "racket9-9.3.1-1.2.cached.openeuler2403.aarch64.rpm")
       (check-contains text "racket9-9.3.1-1.1.postinstall.openeuler2403.aarch64.rpm")
    ...
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (combined-output out err)
                       "--rpm-system must be one of el9, fc40, fc43, fc44, openeuler2203, openeuler2403")
       (check-false (directory-exists? (build-path tmp "artifacts")))
       (check-false (directory-exists? (build-path tmp "work")))
      ) ; end lambda temp dir
    ) ; end with-temp-dir
  ...
  (test-case "rpm-spec writes spec sources and scripts only"
    (with-temp-dir
     (lambda (tmp)
       ...
       (define artifact-dir (build-path tmp "artifacts"))
       (write-text! (build-path artifact-dir "racket-minimal-9.3.1-src.tgz") "fake source artifact")
       (define config-path (build-path tmp "rpm-repo-config.rktd"))
       (write-rpm-repo-config! config-path rpm-repo-root)
       (define-values (out err)
         (run-package/success
       ...
       (define spec-path (build-path rpm-repo-root "SPECS" "racket9-cached.spec"))
       (define postinstall-spec-path
         (build-path rpm-repo-root "SPECS" "racket9-postinstall.spec"))
       ...
       (define postinstall-spec-content (file->string postinstall-spec-path))
       (check-contains spec-content "%global cache_mode cached")
       (check-contains spec-content "%global package_system openeuler2403")
       (check-contains spec-content "%global package_release 1")
       (check-contains spec-content "Name: racket9")
       (check-contains spec-content "Release: %{package_release}.2.cached.%{package_system}")
       (check-contains spec-content "Provides: racket9(cache-mode-cached)")
       (check-contains spec-content "Obsoletes: racket9-cached < %{version}-%{package_release}")
       (check-contains postinstall-spec-content "%global cache_mode postinstall")
       (check-contains postinstall-spec-content "Name: racket9")
       ...
       (check-contains spec-content "Source0: https://github.com/CutieDeng/racket/releases/download/v9.3.1/racket-minimal-9.3.1-src.tgz")
       ...
       (check-contains spec-content "%setup -q -n racket-9.3.1")
       ...
       (check-contains spec-content "add_runtime_link")
       ...
       (check-contains spec-content "-U -R \"$runtime_cache_root\" -X \"$runtime_collects_dir\" -G \"$setup_config_dir\" -N raco -l- raco setup")
       (check-contains spec-content "--no-launcher")
       (check-contains spec-content "-N rhombus -l- rhombus/run.rhm --version")
       (check-contains spec-content "racket -U -R \"$compiled_cache_root\"")
       (check-contains spec-content "package-racket-rhombus-cache")
       (check-contains spec-content "cleanup_rhombus_ephemeral")
       (check-contains spec-content "Rhombus ephemeral cache must not be packaged")
       (check-contains spec-content "runtime_pkgs_dir")
       (check-contains spec-content "move_cache_tree")
       (check-contains spec-content "runtime-keyed staged system compiled cache")
       (check-contains spec-content "runtime-keyed staged package compiled cache")
       (check-contains spec-content "racket-compiled-cache.log")
       (check-contains spec-content "could not prepare isolated Racket setup config")
       (check-contains spec-content "mktemp returned an empty Racket setup config directory")
       (check-contains spec-content "mktemp returned an empty Racket smoke-test home directory")
       (check-contains spec-content "isolated setup config did not select the immutable cache root")
       (check-contains spec-content "isolated setup config did not select the dynamic cache root")
       (check-contains spec-content "setup leaked compiled files into the staged runtime tree")
       (check-contains spec-content "rm -rf \"$staged_cache_root\"")
       (check-contains spec-content "\"$racket_bin\" -U -R \"$runtime_cache_root\" -X \"$runtime_collects_dir\" -G \"$setup_config_dir\"")
       (check-contains spec-content "%posttrans")
       (check-contains spec-content "cleanup_posttrans")
       (check-not-contains spec-content "setup_jobs")
       (check-contains spec-content "rm -rf \"$compiled_cache_root\"")
       (check-contains spec-content "%{package_prefix}/bin/racket -U -R \"$compiled_cache_root\" -X %{package_prefix}/share/racket/collects -G \"$setup_config_dir\"")
       (check-contains spec-content "%files -f %{name}.files")
       (check-contains spec-content "%postun")
       (check-contains spec-content "%global immutable_cache_root %{package_prefix}/lib/racket/%{version}/compiled-cache")
       (check-contains spec-content "%global dynamic_cache_root /var/cache/racket/%{version}/compiled")
       (check-contains spec-content "rm -rf \"%{dynamic_cache_root}\"")
       (check-not-contains spec-content "Conflicts:")
       ...
       (check-not-contains spec-content "-R same")
       (check-not-contains spec-content "setup_bootstrap_root=")
       (check-not-contains spec-content "--system")
       (check-not-contains spec-content "--reset-cache")
       (check-not-contains spec-content "--unsafe-delete-all")
       (check-not-contains spec-content "rhombus_ephemeral_root=")
       (check-not-contains spec-content "config.rktd.package-racket-final")
       ...
       (check-contains (file->string common-path) "DEFAULT_CACHE_MODE=cached")
       (check-contains (file->string common-path) "materialize_spec")
       (check-contains (file->string build-path*) "rpmbuild -ba")
       (check-contains (file->string build-path*) "--define \"_sysconfdir /etc\"")
       (check-contains (file->string srpm-path) "rpmbuild -bs")
       (check-contains (file->string srpm-path) "--cache-mode")
       (check-contains (file->string srpm-path) "materialize_spec")
       ...
       (check-contains (file->string verify-path) "RPM payload unexpectedly includes Rhombus ephemeral cache")
       (check-contains (file->string verify-path) "cache-mode-$CACHE_MODE")
       (check-contains (file->string verify-path) "RPM scriptlets do not reset the compiled cache before Racket starts")
       (check-contains (file->string verify-path) "RPM scriptlets retain in-process compiled-cache reset options")
       (check-contains (file->string verify-path) "RPM scriptlets do not clean Rhombus ephemeral compiled state")
       (check-not-contains (file->string verify-path) "$PREFIX/bin/raco setup")
       (check-contains (file->string verify-path) "obsolete cross-package lifecycle checks")
       (check-contains (file->string verify-path) "racket compiled cache debug log")
       (check-contains (file->string readme-file) "generated RPM packaging and two-channel repository")
       (check-contains (file->string readme-file) "dnf --refresh distro-sync")
       (check-contains (file->string readme-file) "scripts/build-rpm.sh")
       (for ([system (in-list '("el9" "fc40" "fc43" "fc44" "openeuler2203" "openeuler2403"))])
         (check-true (file-exists? (build-path rpm-repo-root f"racket9-{system}.repo"))
                     f"missing system-specific repo file: {system}"))
       (check-true (directory-exists? (build-path rpm-repo-root "repo")))
       (define repository-content
         (file->string (build-path rpm-repo-root "racket9-openeuler2403.repo")))
       (check-contains repository-content "[cutiedeng-racket-openeuler2403-cached]")
       (check-contains repository-content "/cached/openeuler2403/$basearch")
       (check-contains repository-content "[cutiedeng-racket-openeuler2403-postinstall]")
       (check-contains repository-content "/postinstall/openeuler2403/$basearch")
       ...
       (when bash-bin
         ...
         (define materialized-spec (build-path tmp "racket9-materialized.spec"))
         (run-command! 'rpm-spec-materialize
                       bash-bin
                       (list "-c"
                             "source \"$1\"; materialize_spec \"$2\" \"$3\" cached 11 fc44 /opt/racket"
                             "package-racket-test"
                             (path-arg common-path)
                             (path-arg spec-path)
                             (path-arg materialized-spec)))
         (define materialized-content (file->string materialized-spec))
         (check-contains materialized-content "%global cache_mode cached")
         (check-contains materialized-content "%global package_release 11")
         (check-contains materialized-content "%global package_system fc44")
         (check-contains materialized-content "%global package_prefix /opt/racket")
  ...
  (test-case "rpm-ci dry-run validates matrix config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure RPM CI target count: 16")
       (check-contains text "fc43 x86_64 postinstall as racket9 on ubuntu-24.04 in fedora:43")
       (check-contains text "fc43 x86_64 cached as racket9 on ubuntu-24.04 in fedora:43")
       (check-contains text "fc44 x86_64 cached as racket9 on ubuntu-24.04 in fedora:44")
       (check-contains text "openeuler2203 aarch64 cached as racket9 on ubuntu-24.04-arm in openeuler/openeuler:22.03-lts")
       (check-not-contains text "racket9-cached")
       (check-contains text "openeuler2203 x86_64 postinstall as racket9 on ubuntu-24.04 in openeuler/openeuler:22.03-lts")
  ...
  (test-case "deb-spec dry-run generates no files and does not require racket root"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "DEB package version (cached/default): 9.3.1-1.ubuntu2404.2.cached")
       (check-contains text "DEB package version (postinstall/optional): 9.3.1-1.ubuntu2404.1.postinstall")
  ...
  (test-case "deb-spec writes generated scaffold into a temporary deb repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string common-script) "find_staged_collects_dir")
       (check-contains (file->string common-script) "write_staged_config")
       (check-contains (file->string common-script) "normalize_staged_system_cache")
       (check-contains (file->string common-script) "warm_staged_rhombus_cache")
       (check-contains (file->string common-script) "runtime-keyed staged system compiled cache")
       (check-contains (file->string common-script) "runtime-keyed staged package compiled cache")
       (check-contains (file->string common-script) "staged compiled cache dependency metadata contains buildroot paths")
       (check-contains (file->string common-script) "normalize_staged_rhombus_cache")
       (check-contains (file->string common-script) "rewrite_staged_cache_dep_paths")
       (check-contains (file->string common-script) "require_staged_rhombus_cache_root")
       (check-contains (file->string common-script) "runtime-keyed staged Rhombus demod $cache_kind collects cache")
       (check-contains (file->string common-script) "runtime-keyed staged Rhombus demod $cache_kind package cache")
       (check-contains (file->string common-script) "staged Rhombus demod cache contains buildroot-keyed paths")
       (check-contains (file->string common-script) "cd \"$rhombus_ephemeral_cache\"")
       (check-contains (file->string common-script) "pkgs-dir")
       (check-contains (file->string common-script) "racket-compiled-cache.log")
       (check-contains (file->string common-script) "-X \"$collects_dir\" -G \"$config_dir\"")
       (check-contains (file->string common-script) "raco setup --system --no-user --reset-cache -D --no-pkg-deps --no-launcher")
       ...
       (check-contains (file->string build-script) "package-racket-rhombus-cache")
       (check-contains (file->string build-script) "$DEBIAN_DIR/postrm")
       (check-contains (file->string build-script) "Conflicts: $LEGACY_CACHED_PACKAGE_NAME")
       (check-contains (file->string build-script) "Replaces: $LEGACY_CACHED_PACKAGE_NAME")
       (check-contains (file->string build-script) "Provides: $LEGACY_CACHED_PACKAGE_NAME (= $DEB_VERSION)")
       (check-not-contains (file->string build-script) "OTHER_RACKET_PACKAGE=")
       (check-contains (file->string build-script) "rm -rf /var/cache/racket/compiled")
       (check-contains (file->string build-script) "rhombus-lib/rhombus/private/compiled/ephemeral/demod")
       (check-contains (file->string build-script) "rmdir /usr/share/racket/pkgs/rhombus-lib/rhombus/private/compiled/ephemeral")
       (check-contains (file->string verify-script) "dpkg-deb --field")
       (check-contains (file->string verify-script) "dpkg-deb --ctrl-tarfile")
       (check-contains (file->string verify-script) "DEB postinst does not build the system compiled cache")
       (check-contains (file->string verify-script) "DEB postinst does not warm the Rhombus demod cache")
       (check-contains (file->string verify-script) "DEB postinst does not warm the Rhombus version cache into the system cache")
       (check-contains (file->string verify-script) "DEB prerm does not delete the system compiled cache")
       (check-contains (file->string verify-script) "DEB does not conflict with the legacy split-name cached package")
       (check-contains (file->string verify-script) "DEB does not replace the legacy split-name cached package")
       (check-contains (file->string verify-script) "DEB does not provide the legacy split-name cached package")
       (check-contains (file->string verify-script) "cached DEB payload does not include runtime-keyed collects cache")
       (check-contains (file->string verify-script) "cached DEB payload does not include runtime-keyed package cache")
       (check-contains (file->string verify-script) "cached DEB payload does not include Rhombus demod cache")
       (check-contains (file->string verify-script) "cached DEB payload does not include runtime-keyed Rhombus demod collects cache")
       (check-contains (file->string verify-script) "cached DEB payload does not include runtime-keyed Rhombus demod package cache")
       (check-contains (file->string verify-script) "cached DEB payload does not include runtime-keyed Rhombus demod native collects cache")
       (check-contains (file->string verify-script) "cached DEB payload does not include runtime-keyed Rhombus demod native package cache")
       (check-contains (file->string verify-script) "cached DEB payload includes buildroot-keyed Rhombus demod cache paths")
       (check-contains (file->string verify-script) "DEB postrm does not purge the Rhombus demod cache directory")
       (check-contains (file->string verify-script) "DEB postrm does not remove empty Rhombus ephemeral cache parents")
       (check-not-contains (file->string verify-script) "OTHER_RACKET_PACKAGE")
       (check-contains (file->string verify-script) "racket compiled cache debug log")
       ...
       (when (find-executable-path "bash")
         ...
         (run-command! 'deb-spec-verify-deb-dry-run
                       ...
                       (list (path-arg verify-script)
                             "--deb" (path-arg (build-path tmp "artifacts" "racket9_9.3.1-1.ubuntu2404.2.cached_amd64.deb"))
                             "--deb-system" "ubuntu2404"
                             "--deb-release" "1"
                             "--deb-arch" "amd64"
                             "--dry-run"))
  ...
  (test-case "deb-ci dry-run validates matrix config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure DEB CI target count: 6")
       (check-contains text "ubuntu2204 amd64 postinstall as racket9 on ubuntu-24.04 in ubuntu:22.04")
       (check-contains text "ubuntu2204 amd64 cached as racket9 on ubuntu-24.04 in ubuntu:22.04")
       (check-contains text "ubuntu2404 arm64 postinstall as racket9 on ubuntu-24.04-arm in ubuntu:24.04")
       (check-contains text "ubuntu2404 arm64 cached as racket9 on ubuntu-24.04-arm in ubuntu:24.04")
       (check-not-contains text "racket9-cached")
  ...
  (test-case "deb-ci installs generated workflow into a temporary deb repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains workflow-content "EXPECTED_DEB_COUNT: 6")
       (check-contains workflow-content "cache_mode: 'postinstall'")
       (check-contains workflow-content "cache_mode: 'cached'")
       (check-contains workflow-content "package_name: 'racket9'")
       (check-not-contains workflow-content "package_name: 'racket9-cached'")
       (check-contains workflow-content "ubuntu:22.04")
       (check-contains workflow-content "artifact_name: 'deb-ubuntu2204-amd64-cached'")
       (check-contains workflow-content "artifact_name: 'deb-ubuntu2404-arm64-cached'")
       ...
       (check-contains workflow-content "system compiled cache is empty after DEB install")
       (check-contains workflow-content "runtime-keyed collects cache is empty after DEB install")
       (check-contains workflow-content "runtime-keyed package cache is empty after DEB install")
       (check-contains workflow-content "Rhombus demod cache is empty after DEB install")
       (check-contains workflow-content "deb-empty-home-ok")
       (check-contains workflow-content "timeout 30s rhombus --version")
       (check-contains workflow-content "deb-rhombus-ok")
       (check-contains workflow-content "deb-rhombus-fresh-home-ok")
       (check-contains workflow-content "system compiled cache remains after DEB purge")
       ...
       (check-not-contains workflow-content "${ matrix")
      ) ; end lambda temp dir
    ) ; end with-temp-dir
  ...
  (test-case "rpm-ci installs generated workflow into a temporary rpm repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains workflow-content "test-rpm-transactions:")
       ...
       (check-contains workflow-content "fedora:43")
       (check-contains workflow-content "fedora:44")
       ...
       (check-contains workflow-content "selected compiled cache is empty after RPM install")
       ...
       (check-contains workflow-content "Rhombus ephemeral cache was installed")
       (check-contains workflow-content "rpm-empty-home-ok")
       (check-contains workflow-content "timeout 30s rhombus --version")
       (check-contains workflow-content "rpm-rhombus-ok")
       (check-contains workflow-content "rpm-rhombus-fresh-home-ok")
       (check-contains workflow-content "dynamic compiled cache remains after RPM erase")
       (check-contains workflow-content "immutable compiled cache remains after RPM erase")
       (check-contains workflow-content "rpm -V \"$PACKAGE_NAME\"")
       (check-contains workflow-content "--enablerepo=racket-transaction-postinstall --refresh distro-sync")
       (check-contains workflow-content "rpm -Uvh --replacepkgs")
       (check-contains workflow-content "cached reinstall left stale dynamic cache")
       (check-contains workflow-content "postinstall transaction created a root addon tree")
       (check-contains workflow-content "postinstall transaction created a HOME addon tree")
       (check-contains workflow-content "postinstall transaction created a root compiled cache")
       (check-contains workflow-content "postinstall transaction created a user compiled cache")
       (check-contains workflow-content "Downloaded RPM files")
       (check-contains workflow-content "Release assets before upload")
       (check-contains workflow-content "Release assets after upload")
       (check-contains workflow-content "Deleting stale release asset")
       (check-contains workflow-content "Immutable release tag")
       (check-contains workflow-content "Build system-isolated channel repositories")
       (check-contains workflow-content "Pages repository must contain metadata only")
       (check-contains workflow-content "diff -u \"$expected_assets\" \"$actual_assets\"")
       (check-contains workflow-content "GH_REPO: ${{ github.repository }}")
       (check-contains workflow-content "--repo \"$GITHUB_REPOSITORY\"")
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.3.1.1-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.3.1.1-windows-x86_64-setup.exe")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.3.1.1-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.3.1.1-windows-x86_64-setup.exe")
       ...
       (check-contains workflow-content "SOURCE_URL: 'https://github.com/CutieDeng/racket/releases/download/v9.3.1/racket-minimal-9.3.1-src.tgz'")
       (check-contains workflow-content "SOURCE_SHA256: '")
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.3.1.1-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.3.1.1-windows-x86_64-setup.exe'")
       ...
       (check-contains workflow-content "choco install innosetup")
       (check-contains workflow-content "Build Inno Setup installer")
       (check-contains workflow-content "ISCC.exe")
       (check-contains workflow-content "/CACHEPATH")
       (check-contains workflow-content "raco setup --system --no-user --reset-cache -D --no-pkg-deps")
       (check-contains workflow-content "package-racket-rhombus-cache")
       (check-contains workflow-content "Rhombus demod cache is empty")
       (check-contains workflow-content "RegDeleteKeyIncludingSubkeys")
       (check-contains workflow-content "artifacts/*.exe")
  ...
  (test-case "rpm plus rpm-repo dry-run uses planned rpm output and writes no repo files"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "RPM package release base: 1")
       (check-contains text "RPM package release (cached/default): 1.2.cached.openeuler2403")
       (check-contains text "RPM repo package: racket9-9.3.1-1.2.cached.openeuler2403.aarch64.rpm")
       (check-contains text "RPM repo package: racket9-9.3.1-1.1.postinstall.openeuler2403.aarch64.rpm")
       ...
       (check-contains text "repo/postinstall/openeuler2403/aarch64/Packages")
     ...
     (lambda (tmp)
       ...
       (define-values (out err)
         (run-package/success
          (list "--target" "apt"
                "--racket-root" (path-arg racket-root)
                "--formula-version" "9.3.1.1"
                "--artifact-dir" (path-arg artifact-dir)
                "--work-dir" (path-arg work-dir)
                "--dry-run")))
       (define text (combined-output out err))
       (check-contains text "Formula/package version: 9.3.1.1")
       (check-contains text "Racket source version: 9.3.1")
       (check-contains text "racket9_9.3.1.1-1_amd64.deb")
       (check-false (directory-exists? artifact-dir))
      ) ; end lambda temp dir
    ) ; end with-temp-dir
  ...
  (test-case "apt-release dry-run validates an existing deb and does not read token"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define deb-name "racket9_9.3.1-1_amd64.deb")
  ...
  (test-case "apt plus apt-release dry-run uses planned apt output"
    (with-temp-dir
     (lambda (tmp)
       ...
       (define artifact-dir (build-path tmp "artifacts"))
       (define work-dir (build-path tmp "work"))
       (define deb-name "racket9_9.3.1.1-1_amd64.deb")
       (define config-path (build-path tmp "apt-release-config.rktd"))
       (write-apt-release-config! config-path deb-name)
       (define-values (out err)
         (run-package/success
  ...
  (test-case "apt-release derives tag and asset from formula-version"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "APT package:")
       (check-contains text "racket9_9.3.1.1-1_amd64.deb")
       (check-contains text "APT release tag: v9.3.1")
       (check-contains text "APT release asset: racket9_9.3.1.1-1_amd64.deb")
       (check-contains text "Would upload apt release asset from planned apt output")
      ) ; end lambda temp dir
    ) ; end with-temp-dir
  ...
  (test-case "source-release dry-run validates an existing tgz without racket root"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define asset-name "racket-minimal-9.3.1-src.tgz")
       ...
       (define-values (out err)
         (run-package/success
          (list "--target" "brew"
                ...
                "--bottle-root-url" "https://github.com/CutieDeng/homebrew-racket/releases/download/v9.3.1"
                "--artifact-dir" (path-arg artifact-dir)
                "--work-dir" (path-arg work-dir)
                "--source-release-config" (path-arg config-path)
                "--dry-run")))
     ...
     (lambda (tmp)
       ...
       (define-values (out err)
         (run-package/success
          (list "--target" "brew-ci"
                "--racket-root" (path-arg racket-root)
                "--formula-version" "9.3.1"
                "--homebrew-tap" (path-arg tap-dir)
                "--bottle-root-url" "https://github.com/CutieDeng/homebrew-racket/releases/download/v9.3.1"
                "--work-dir" (path-arg work-dir)
                "--brew-ci-config" (path-arg config-path))))
       (define publish-yml (build-path tap-dir ".github" "workflows" "publish.yml"))
       (check-contains (combined-output out err) "Formula/package version: 9.3.1")
       (check-contains (file->string publish-yml) "RELEASE_TAG: v9.3.1")
      ) ; end lambda temp dir
    ) ; end with-temp-dir
