  ...
  (test-case "deb-spec writes generated scaffold into a temporary deb repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string common-script) "raco setup --system --no-user --compiled-cache-root \"$staged_cache_root\" --reset-cache -D --no-pkg-deps --no-launcher")
       (check-contains (file->string common-script) "PLTCOMPILEDROOTS")
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.3.5.1-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.3.5.1-windows-x86_64-setup.exe")
       (check-contains text "Would configure Windows cached Inno installer: racket9-9.3.5.1-windows-x86_64-setup-cached.exe")
       (check-contains text "Would configure Windows cached portable zip: racket9-9.3.5.1-windows-x86_64-cached.zip")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.3.5.1-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.3.5.1-windows-x86_64-setup.exe")
       (check-contains (file->string readme-file) "racket9-9.3.5.1-windows-x86_64-setup-cached.exe")
       (check-contains (file->string readme-file) "racket9-9.3.5.1-windows-x86_64-cached.zip")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.3.5.1-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.3.5.1-windows-x86_64-setup.exe'")
       (check-contains workflow-content "CACHED_EXE_NAME: 'racket9-9.3.5.1-windows-x86_64-setup-cached.exe'")
       (check-contains workflow-content "CACHED_ZIP_NAME: 'racket9-9.3.5.1-windows-x86_64-cached.zip'")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.3.5.1-windows-arm64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.3.5.1-windows-arm64-setup.exe'")
       (check-contains workflow-content "CACHED_EXE_NAME: 'racket9-9.3.5.1-windows-arm64-setup-cached.exe'")
  ...
  (test-case "apt plus apt-release dry-run uses planned apt output"
    (with-temp-dir
     (lambda (tmp)
       ...
       (define artifact-dir (build-path tmp "artifacts"))
       (define work-dir (build-path tmp "work"))
       (define deb-name "racket9_9.3.5.1-1_amd64.deb")
       (define config-path (build-path tmp "apt-release-config.rktd"))
       (write-apt-release-config! config-path deb-name)
       (define-values (out err)
         (run-package/success
