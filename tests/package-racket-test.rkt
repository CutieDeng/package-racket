  ...
  (test-case "deb-spec dry-run generates no files and does not require racket root"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "DEB package version: 9.2.5-1.ubuntu2404")
  ...
  (test-case "deb-spec writes generated scaffold into a temporary deb repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string build-script) "package_present")
       (check-contains (file->string build-script) "other_racket_package_present")
       (check-contains (file->string build-script) "OTHER_RACKET_PACKAGE=")
       (check-contains (file->string build-script) "db:Status-Abbrev")
       ...
       (check-contains (file->string verify-script) "DEB prerm does not guard cache deletion for package replacement")
       ...
       (check-contains (file->string verify-script) "DEB postrm does not guard shared cache deletion for package replacement")
       ...
       (when (find-executable-path "bash")
         ...
         (run-command! 'deb-spec-verify-deb-dry-run
                       ...
                       (list (path-arg verify-script)
                             "--deb" (path-arg (build-path tmp "artifacts" "racket9_9.2.5-1.ubuntu2404_amd64.deb"))
                             "--deb-system" "ubuntu2404"
                             "--deb-release" "1"
                             "--deb-arch" "amd64"
                             "--dry-run"))
  ...
  (test-case "deb-ci dry-run validates matrix config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "ubuntu2204 amd64 cached as racket9-cached on ubuntu-24.04 in ubuntu:22.04")
       (check-contains text "ubuntu2404 arm64 postinstall as racket9 on ubuntu-24.04-arm in ubuntu:24.04")
       (check-contains text "ubuntu2404 arm64 cached as racket9-cached on ubuntu-24.04-arm in ubuntu:24.04")
