...
(module+ test
  ...
  (test-case "apt dry-run is isolated from apt-release config and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "racket9_9.2.2.5-1_amd64.deb")
  ...
  (test-case "rpm arm64 dry-run normalizes to aarch64 and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Formula/package version: 9.2.2.5")
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.2.2.5-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.2.2.5-windows-x86_64-setup.exe")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.2.2.5-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.2.2.5-windows-x86_64-setup.exe")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.2.2.5-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.2.2.5-windows-x86_64-setup.exe'")
  ...
  (test-case "apt-release dry-run validates an existing deb and does not read token"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define deb-name "racket9_9.2.2.5-1_amd64.deb")
