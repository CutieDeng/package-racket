  ...
  (test-case "apt dry-run is isolated from apt-release config and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "racket9_9.2.4.2-1_amd64.deb")
  ...
  (test-case "rpm arm64 dry-run normalizes to aarch64 and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Formula/package version: 9.2.4.2")
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.2.4.2-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.2.4.2-windows-x86_64-setup.exe")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.2.4.2-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.2.4.2-windows-x86_64-setup.exe")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.2.4.2-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.2.4.2-windows-x86_64-setup.exe'")
  ...
  (test-case "apt plus apt-release dry-run uses planned apt output"
    (with-temp-dir
     (lambda (tmp)
       ...
       (define artifact-dir (build-path tmp "artifacts"))
       (define work-dir (build-path tmp "work"))
       (define deb-name "racket9_9.2.4.2-1_amd64.deb")
       (define config-path (build-path tmp "apt-release-config.rktd"))
       (write-apt-release-config! config-path deb-name)
       (define-values (out err)
         (run-package/success
  ...
  (test-case "apt-release derives tag and asset from formula-version"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "APT release asset: racket9_9.2.4.2-1_amd64.deb")
       (check-contains text "Would upload apt release asset from planned apt output")
      ) ; end lambda temp dir
    ) ; end with-temp-dir
