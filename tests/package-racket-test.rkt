  ...
  (test-case "rpm-spec writes spec sources and scripts only"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains spec-content "-U -R \"$runtime_cache_root\"")
       ...
       (check-contains spec-content "could not prepare Racket setup bootstrap config")
       (check-contains spec-content "mktemp returned an empty Racket setup config directory")
       (check-contains spec-content "setup bootstrap config did not select the immutable cache root")
       (check-contains spec-content "setup bootstrap config did not select the dynamic cache root")
       (check-contains spec-content "setup bootstrap leaked compiled files into the staged runtime tree")
       (check-contains spec-content "setup_bootstrap_root=\"$setup_config_dir/compiled\"")
       (check-contains spec-content "\"$racket_bin\" -U -R \"$setup_bootstrap_root\" -X \"$runtime_collects_dir\" -G \"$setup_config_dir\"")
       ...
       (check-contains spec-content "%{package_prefix}/bin/racket -U -R \"$setup_bootstrap_root\" -X %{package_prefix}/share/racket/collects -G \"$setup_config_dir\"")
       (check-contains spec-content "--reset-cache --unsafe-delete-all")
       ...
       (check-contains (file->string verify-path) "RPM scriptlets do not isolate setup from the cache being reset")
       (check-contains (file->string verify-path) "RPM scriptlets do not keep bootstrap bytecode under the temporary setup directory")
