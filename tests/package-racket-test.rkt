...
(module+ test
  ...
  (test-case "apt dry-run is isolated from apt-release config and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "racket9_9.2.2.6-1_amd64.deb")
  ...
  (test-case "rpm arm64 dry-run normalizes to aarch64 and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Formula/package version: 9.2.2.6")
       ...
       (check-contains text "racket9-9.2.2-1.openeuler2403.aarch64.rpm")
  ...
  (test-case "rpm-spec writes spec sources and scripts only"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains spec-content "raco setup --system --no-user --reset-cache -D --no-pkg-deps")
       ...
       (check-contains (file->string verify-path) "cached RPM payload does not include Rhombus demod cache")
       (check-contains (file->string verify-path) "RPM scriptlets do not warm the Rhombus demod cache")
       (check-contains (file->string verify-path) "RPM scriptlets do not warm the Rhombus version cache into the system cache")
       (check-contains (file->string verify-path) "RPM preun does not guard cache deletion for package replacement")
       (check-contains (file->string verify-path) "RPM scriptlets do not purge the Rhombus demod cache directory")
       (check-contains (file->string verify-path) "RPM scriptlets do not remove empty Rhombus ephemeral cache parents")
       (check-contains (file->string verify-path) "RPM postun does not guard shared cache deletion for package replacement")
  ...
  (test-case "rpm-ci dry-run validates matrix config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "fc43 x86_64 cached as racket9-cached on ubuntu-24.04 in fedora:43")
       (check-contains text "fc44 x86_64 cached as racket9-cached on ubuntu-24.04 in fedora:44")
       (check-contains text "openeuler2203 aarch64 cached as racket9-cached on ubuntu-24.04-arm in openeuler/openeuler:22.03-lts")
  ...
  (test-case "rpm-ci installs generated workflow into a temporary rpm repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains workflow-content "Rhombus demod cache is empty after RPM install")
       ...
       (check-contains workflow-content "system compiled cache remains after RPM erase")
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.2.2.6-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.2.2.6-windows-x86_64-setup.exe")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.2.2.6-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.2.2.6-windows-x86_64-setup.exe")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.2.2.6-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.2.2.6-windows-x86_64-setup.exe'")
  ...
  (test-case "rpm plus rpm-repo dry-run uses planned rpm output and writes no repo files"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "RPM package release base: 1")
       (check-contains text "RPM package release: 1.openeuler2403")
       (check-contains text "RPM repo package: racket9-9.2.2-1.openeuler2403.aarch64.rpm")
  ...
  (test-case "apt-release dry-run validates an existing deb and does not read token"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define deb-name "racket9_9.2.2.6-1_amd64.deb")
