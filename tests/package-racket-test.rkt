...
(define (make-fake-homebrew-tap! base)
  ...
  test do
    assert_match \"9.3.4\", shell_output(\"racket -v\")
  end
end
")
    tap
...
"))
  ...
  (define (write-rpm-ci-config! path)
    (write-text!
     path
     "#hash((release-tag . \"v9.3.4\")
      (release-name . \"Racket 9.3.4 RPM packages\")
      (artifact-prefix . \"rpm\")
      (create-release . #t)
  ...
  (define (write-windows-ci-config! path root #:publish-release? [publish-release? #t])
    (write-text!
     ...
     f"#hash((windows-repo-root . \"{(path-arg root)}\")
      ...
      (release-tag . \"v9.3.4\")
      (release-name . \"Racket 9.3.4 Windows portable\")
      (create-release . #f)
...
(module+ test
  ...
  (test-case "apt dry-run is isolated from apt-release config and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "racket9_9.3.4.1-1_amd64.deb")
  ...
  (test-case "rpm arm64 dry-run normalizes to aarch64 and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Formula/package version: 9.3.4.1")
       (check-contains text "RPM target arch: aarch64")
       (check-contains text "RPM target system: openeuler2403")
       (check-contains text "RPM package version: 9.3.4")
       ...
       (check-contains text "RPM source archive: https://github.com/CutieDeng/racket/releases/download/v9.3.4/racket-minimal-9.3.4-src.tgz")
       (check-contains text "racket9-9.3.4-1.2.cached.openeuler2403.aarch64.rpm")
       (check-contains text "racket9-9.3.4-1.1.postinstall.openeuler2403.aarch64.rpm")
  ...
  (test-case "rpm-spec writes spec sources and scripts only"
    (with-temp-dir
     (lambda (tmp)
       ...
       (define artifact-dir (build-path tmp "artifacts"))
       (write-text! (build-path artifact-dir "racket-minimal-9.3.4-src.tgz") "fake source artifact")
       (define config-path (build-path tmp "rpm-repo-config.rktd"))
       (write-rpm-repo-config! config-path rpm-repo-root)
       (define-values (out err)
         (run-package/success
       ...
       (check-contains spec-content "Source0: https://github.com/CutieDeng/racket/releases/download/v9.3.4/racket-minimal-9.3.4-src.tgz")
       ...
       (check-contains spec-content "%setup -q -n racket-9.3.4")
  ...
  (test-case "deb-spec dry-run generates no files and does not require racket root"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "DEB package version (cached/default): 9.3.4-1.ubuntu2404.2.cached")
       (check-contains text "DEB package version (postinstall/optional): 9.3.4-1.ubuntu2404.1.postinstall")
       ...
       (when (find-executable-path "bash")
         ...
         (run-command! 'deb-spec-verify-deb-dry-run
                       ...
                       (list (path-arg verify-script)
                             "--deb" (path-arg (build-path tmp "artifacts" "racket9_9.3.4-1.ubuntu2404.2.cached_amd64.deb"))
                             "--deb-system" "ubuntu2404"
                             "--deb-release" "1"
                             "--deb-arch" "amd64"
                             "--dry-run"))
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.3.4.1-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.3.4.1-windows-x86_64-setup.exe")
       (check-contains text "Would configure Windows cached Inno installer: racket9-9.3.4.1-windows-x86_64-setup-cached.exe")
       (check-contains text "Would configure Windows cached portable zip: racket9-9.3.4.1-windows-x86_64-cached.zip")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.3.4.1-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.3.4.1-windows-x86_64-setup.exe")
       (check-contains (file->string readme-file) "racket9-9.3.4.1-windows-x86_64-setup-cached.exe")
       (check-contains (file->string readme-file) "racket9-9.3.4.1-windows-x86_64-cached.zip")
       ...
       (check-contains workflow-content "SOURCE_URL: 'https://github.com/CutieDeng/racket/releases/download/v9.3.4/racket-minimal-9.3.4-src.tgz'")
       (check-contains workflow-content "SOURCE_SHA256: '")
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.3.4.1-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.3.4.1-windows-x86_64-setup.exe'")
       (check-contains workflow-content "CACHED_EXE_NAME: 'racket9-9.3.4.1-windows-x86_64-setup-cached.exe'")
       (check-contains workflow-content "CACHED_ZIP_NAME: 'racket9-9.3.4.1-windows-x86_64-cached.zip'")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.3.4.1-windows-arm64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.3.4.1-windows-arm64-setup.exe'")
       (check-contains workflow-content "CACHED_EXE_NAME: 'racket9-9.3.4.1-windows-arm64-setup-cached.exe'")
  ...
  (test-case "rpm plus rpm-repo dry-run uses planned rpm output and writes no repo files"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "RPM package release base: 1")
       (check-contains text "RPM package release (cached/default): 1.2.cached.openeuler2403")
       (check-contains text "RPM repo package: racket9-9.3.4-1.2.cached.openeuler2403.aarch64.rpm")
       (check-contains text "RPM repo package: racket9-9.3.4-1.1.postinstall.openeuler2403.aarch64.rpm")
     ...
     (lambda (tmp)
       ...
       (define-values (out err)
         (run-package/success
          (list "--target" "apt"
                "--racket-root" (path-arg racket-root)
                "--formula-version" "9.3.4.1"
                "--artifact-dir" (path-arg artifact-dir)
                "--work-dir" (path-arg work-dir)
                "--dry-run")))
       (define text (combined-output out err))
       (check-contains text "Formula/package version: 9.3.4.1")
       (check-contains text "Racket source version: 9.3.4")
       (check-contains text "racket9_9.3.4.1-1_amd64.deb")
       (check-false (directory-exists? artifact-dir))
      ) ; end lambda temp dir
    ) ; end with-temp-dir
  ...
  (test-case "apt-release dry-run validates an existing deb and does not read token"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define deb-name "racket9_9.3.4-1_amd64.deb")
  ...
  (test-case "apt plus apt-release dry-run uses planned apt output"
    (with-temp-dir
     (lambda (tmp)
       ...
       (define artifact-dir (build-path tmp "artifacts"))
       (define work-dir (build-path tmp "work"))
       (define deb-name "racket9_9.3.4.1-1_amd64.deb")
       (define config-path (build-path tmp "apt-release-config.rktd"))
       (write-apt-release-config! config-path deb-name)
       (define-values (out err)
         (run-package/success
  ...
  (test-case "apt-release derives tag and asset from formula-version"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "APT package:")
       (check-contains text "racket9_9.3.4.1-1_amd64.deb")
       (check-contains text "APT release tag: v9.3.4")
       (check-contains text "APT release asset: racket9_9.3.4.1-1_amd64.deb")
       (check-contains text "Would upload apt release asset from planned apt output")
      ) ; end lambda temp dir
    ) ; end with-temp-dir
  ...
  (test-case "source-release dry-run validates an existing tgz without racket root"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define asset-name "racket-minimal-9.3.4-src.tgz")
       ...
       (define-values (out err)
         (run-package/success
          (list "--target" "brew"
                ...
                "--bottle-root-url" "https://github.com/CutieDeng/homebrew-racket/releases/download/v9.3.4"
                "--artifact-dir" (path-arg artifact-dir)
                "--work-dir" (path-arg work-dir)
                "--source-release-config" (path-arg config-path)
                "--dry-run")))
     ...
     (lambda (tmp)
       ...
       (define-values (out err)
         (run-package/success
          (list "--target" "brew-ci"
                "--racket-root" (path-arg racket-root)
                "--formula-version" "9.3.4"
                "--homebrew-tap" (path-arg tap-dir)
                "--bottle-root-url" "https://github.com/CutieDeng/homebrew-racket/releases/download/v9.3.4"
                "--work-dir" (path-arg work-dir)
                "--brew-ci-config" (path-arg config-path))))
       (define publish-yml (build-path tap-dir ".github" "workflows" "publish.yml"))
       (check-contains (combined-output out err) "Formula/package version: 9.3.4")
       (check-contains (file->string publish-yml) "RELEASE_TAG: v9.3.4")
      ) ; end lambda temp dir
    ) ; end with-temp-dir
