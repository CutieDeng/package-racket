...
(module+ test
  ...
  (test-case "apt dry-run is isolated from apt-release config and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "racket9_9.2.2.2-1_amd64.deb")
  ...
  (test-case "rpm arm64 dry-run normalizes to aarch64 and writes no artifacts"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Formula/package version: 9.2.2.2")
  ...
  (test-case "rpm-spec writes spec sources and scripts only"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains spec-content "-X \"$collects_dir\" -G \"$config_dir\"")
       (check-contains spec-content "rpm-staged-config")
       (check-contains spec-content "pkgs-dir")
  ...
  (test-case "deb-ci installs generated workflow into a temporary deb repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains workflow-content "rhombus --version")
       (check-contains workflow-content "deb-rhombus-ok 1")
  ...
  (test-case "rpm-ci installs generated workflow into a temporary rpm repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains workflow-content "rhombus --version")
       (check-contains workflow-content "rpm-rhombus-ok 1")
  ...
  (test-case "windows-portable-ci dry-run validates config and writes no workflow"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains text "Would configure Windows portable zip: racket9-9.2.2.2-windows-x86_64.zip")
       (check-contains text "Would configure Windows Inno installer: racket9-9.2.2.2-windows-x86_64-setup.exe")
  ...
  (test-case "windows-portable-ci installs generated workflow into configured repository"
    (with-temp-dir
     (lambda (tmp)
       ...
       (check-contains (file->string readme-file) "racket9-9.2.2.2-windows-x86_64.zip")
       (check-contains (file->string readme-file) "racket9-9.2.2.2-windows-x86_64-setup.exe")
       ...
       (check-contains workflow-content "ZIP_NAME: 'racket9-9.2.2.2-windows-x86_64.zip'")
       (check-contains workflow-content "EXE_NAME: 'racket9-9.2.2.2-windows-x86_64-setup.exe'")
  ...
  (test-case "apt-release dry-run validates an existing deb and does not read token"
    (with-temp-dir
     (lambda (tmp)
       (define artifact-dir (build-path tmp "artifacts"))
       (define deb-name "racket9_9.2.2.2-1_amd64.deb")
